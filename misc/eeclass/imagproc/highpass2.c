From rayssd!SUD2.ED.RAY.COM!LINGARD@uunet.UU.NET Mon Oct 18 18:59:10 1993
Received: from relay1.UU.NET (relay1.UU.NET [192.48.96.5]) by bigboote (8.6.2/8.6) with SMTP id SAA28684 for <lingard@wpi.edu>; Mon, 18 Oct 1993 18:59:08 -0400
Received: from spool.uu.net (via LOCALHOST) by relay1.UU.NET with SMTP 
	(5.61/UUNET-internet-primary) id AA10046; Mon, 18 Oct 93 18:59:07 -0400
Received: from rayssd.UUCP by uucp4.uu.net with UUCP/RMAIL
	(queueing-rmail) id 185738.24382; Mon, 18 Oct 1993 18:57:38 EDT
Received: from SUD2.ED.RAY.COM by rayssd.ssd.ray.com with SMTP id AA19349
  (5.65c/IDA-1.5 for <lingard@wpi.edu>); Mon, 18 Oct 1993 18:12:05 -0400
Received: from SUD2.ED.RAY.COM by SUD2.ED.RAY.COM (PMDF #2325 ) id
 <01H49MPS7XOG8WWXJ0@SUD2.ED.RAY.COM>; Mon, 18 Oct 1993 18:13:11 EDT
Date: 18 Oct 1993 18:13:11 -0400 (EDT)
From: LINGARD@SUD2.ED.RAY.COM
To: lingard@wpi.edu
Message-Id: <01H49MPS87BM8WWXJ0@SUD2.ED.RAY.COM>
...
if (DEBUG == 1) printf("lowpass: allocated mem ptr's\n");
...
if (DEBUG == 1) printf("lowpass: setup out image parameters\n");
...
if (DEBUG == 1) printf("lowpass: copied f into z\n");
...
if (DEBUG == 1) printf("lowpass: init top and bottom rows\n");
...
if (DEBUG == 1) printf("lowpass: init left and right columns\n");
...
if (DEBUG == 1) printf("highpass: assigned low ptr\n");
...
if (DEBUG == 1) printf("highpass: alloc mem for out image\n");
	...
	img_out->ydim = NY;
	img_out->bits = out_ptr;
if (DEBUG == 1) printf("highpass: setup out img\n");

     /* perform highpass = original - lowpass */
	for (x = 0; x <= NX - 1; x++) {
if (DEBUG == 1) printf("x = %d\n",x);
	  for (y = 0; y <= NY - 1; y++) {
/*if (DEBUG == 1) printf("x = %d, y = %d\n",x,y);*/
