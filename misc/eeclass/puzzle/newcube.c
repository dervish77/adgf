...
/*      InitPESocks();*/
      ...
      if (prob.numprocs > 1) {	/* multiple PEs */
         ...
         NewDistributeCube();		/* distribute cube to other PEs */
         ...
         switch(fork()) {
            ...
            default:	/* parent process */
               while(!donesolving) { /* recv cmds 'til all slaves done */ 
                  ...
                  if (pesdone == SLAVESDONE) { /* if all slaves done, perform cleanup */
                     for (i=1; i<prob.numprocs; i++) {
...
/*if (DEBUG==TRUE) fprintf(logfile, "pesdone = %x\n", pesdone);*/
/*if (DEBUG==TRUE) printf("pesdone = %x\n", pesdone);*/
            break;
