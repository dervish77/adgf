...
# unlocklabel
#
#	Unlocks specified label in all VOBs within current view.  
#
# Usage:
#	unlocklabel <viewname> <label>
#
# Options:
#	<viewname>	- name of a ClearCase view {verified to exist}
#
#	<label>		- label to unlock {verified to exist}
#
# Notes:
...
if [ $# -lt 2 ]
then
  echo "Usage: unlocklabel <viewname> <label>"
  echo "(i.e.  unlocklabel main REL_SMM_PROD_01.03.03)"
  exit 1
else
  VIEWNAME=$1
  LABEL=$2
...
then
  echo "Unlocking label $LABEL in view $VIEWTAG"

  #
  # unlock label in each VOB in VOBLIST
  #
  for VOB in $VOBLIST
  do
    cd $VOB
    ...
    then
      echo "Unlocking label $LABEL in $VOB ..."
      $CTL unlock lbtype:$LABEL
    else
