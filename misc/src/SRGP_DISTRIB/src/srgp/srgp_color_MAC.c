...
/* The original SRGP for the Mac used color tables directly instead
   of the Palette Manager.  Using the PM makes the code much simpler
   and friendlier with the color environment. */
...
*/
   for (i=startentry,j=0; j<count; i++,j++){
      ...
      rgb.green = greeni[j];
      rgb.blue = bluei[j];
	  SetEntryColor(pal, i, &rgb);
   }	
/*	ActivatePalette(FrontWindow());*/
	srgp__canvasTable[0].palette_updated = TRUE;
#endif
}
...
SRGP__initColor (int requested_planes)
{
   ...
   
   {
	 ...
	 srgp__application_depth = requested_planes;
      ...
      /* the following takes care of the case that srgp__available_depth >= 32 */
      if(actual__max_pixel_value==-1) actual__max_pixel_value = 1000;
      
	pal = NewPalette(actual__max_pixel_value+1, NULL, pmTolerant, 0x0000);
