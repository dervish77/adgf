...
void SRGP__initGraphicsDevice (char *name, int requested_planes, boolean debugasap)
{
   ...
   srgpmac__cwindow = 
      NewCWindow (0L, &windowBounds, name, true, noGrowDocProc, (WindowPtr)-1L, true, 0);
#else
   srgpmac__cwindow = 
      NewWindow (0L, &windowBounds, name, true, noGrowDocProc, (WindowPtr)-1L, true, 0);
#endif
...
#ifdef COLOR_QUICKDRAW
   SetPalette(srgpmac__cwindow, SRGP__getPalette(), TRUE);
#endif   
   ...
   /* Load the menus in the menu bar. */	
   ClearMenuBar();
   InsertMenu (GetMenu(128), 0);
   AddResMenu(GetMHandle(128),'DRVR');        /*add items to Apple menu*/
   InsertMenu (srgpmac__srgpmenu=GetMenu(129), 0);
