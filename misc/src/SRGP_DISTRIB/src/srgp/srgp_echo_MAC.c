...
SRGP__disableLocatorRubberEcho()
{
   ...
   if(FrontWindow()!=srgp__curActiveCanvasSpec.drawable.win) 
      SelectWindow(srgp__curActiveCanvasSpec.drawable.win);
   if(srgp__canvasTable[0].palette_updated == TRUE) {
      ActivatePalette(srgp__curActiveCanvasSpec.drawable.win);
      srgp__canvasTable[0].palette_updated = FALSE;
   }
}
