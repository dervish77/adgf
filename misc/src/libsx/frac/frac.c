 ...
 * we will #define it if it isn't (we check later on to make sure that
 * we have a reasonable value).
 */
#ifndef RAND_MAX
...
main(int argc, char **argv)
{
  ...
  /* initialize */
  srandom((getpid()*time(NULL)) | 0x01); 

  /*
   * We do this little ditty here because sometimes RAND_MAX is defined
   * but rand() and random() return different ranges of values.  Since
   * random() is almost always based on the berkely random() function,
   * we assume that if we get a number bigger than RAND_MAX that the maximum
   * value we really should be using for max_random is 0x7ffffff (which
   * is the largest signed 32-bit integer).
   */
  for(a=0; a < 50; a++)
    if (random() > RAND_MAX)
     {
...
fracline(int a, int b)
{
  ...
  if ( (b - a) > 1){
    ...
    temp2 = (double)random() / (double)max_random;
    temp2 = (temp2 * 2.0) - 1.0;
