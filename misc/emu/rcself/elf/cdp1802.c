...
void cdp1802Cycle(CDP1802* cpu) {
  ...
  switch (cpu->i) {
    case 0: if (cpu->n == 0) { cpu->idle=1;
              ...
              if (trace == 'Y') {
                sprintf(buffer," LDN %x     D=%x",cpu->n,cpu->d);
                strcat(tr,buffer);
                }
              }
            break;
    case 1: cpu->r[cpu->n]++;
            if (trace == 'Y') {
              sprintf(buffer," INC %x    R[%x]=%x",cpu->n,cpu->n,cpu->r[cpu->n]);
              strcat(tr,buffer);
              }
            break;
    case 2: cpu->r[cpu->n]--;
            if (trace == 'Y') {
              sprintf(buffer," DEC %x    R[%x]=%x",cpu->n,cpu->n,cpu->r[cpu->n]);
              strcat(tr,buffer);
              }
            break;
    case 3: switch (cpu->n) {
    ...
    case 6: if (cpu->n == 0) {
            ...
            if (cpu->n <= 7) {
              ...
              cpu->writePort(cpu->n,cpu->readMem(cpu->r[cpu->x]++));
              if (trace == 'Y') {
                sprintf(buffer," OUT %X ",cpu->n);
                strcat(tr,buffer);
                }
              break;
              }
    ...
    case 10: cpu->r[cpu->n]=(cpu->r[cpu->n] & 0xff00) | cpu->d;
            if (trace == 'Y') {
              sprintf(buffer," PLO %X       R%X = %X ",cpu->n,cpu->n,cpu->r[cpu->n]);
              strcat(tr,buffer);
              }
            break;
    case 11: cpu->r[cpu->n]=(cpu->r[cpu->n] & 0x00ff) | (cpu->d<<8);
            if (trace == 'Y') {
              sprintf(buffer," PHI %X       R%X = %X ",cpu->n,cpu->n,cpu->r[cpu->n]);
              strcat(tr,buffer);
              }
            break;
