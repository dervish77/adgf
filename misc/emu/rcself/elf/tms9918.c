...
TMS9918 tms9918_init() {
  ...
  TMS9918 vdp;
  if (useZoom == 'Y')
    vdp.window=rcs_createWindow(display,rcs_rootWindow(display),0,0,512,384);
  else
    vdp.window=rcs_createWindow(display,rcs_rootWindow(display),0,0,256,192);
    ...
    case 0:c = vdp->mem[vdp->colorAddr + (p >> 3)];
           ...
           for (py=0; py<8; py++) {
             ...
             for (px=0; px<8; px++) {
               ...
               if (cl == 0) cl = 1;
               if (useZoom == 'Y') 
                 rcs_drawFilledBox(display,vdp->window,vdp->colors[cl],x*16+(px<<1),y*16+(py<<1),2,2);
               else
                 rcs_drawPoint(display,vdp->window,vdp->colors[cl],x*8+px,y*8+py);
               b = (b << 1) & 0xff;
               }
             }
           break;
    ...
    case 4:c = vdp->regs[7];
           ...
           for (py=0; py<8; py++) {
             ...
             for (px=0; px<6; px++) {
               ...
               if (cl == 0) cl = 1;
               if (useZoom == 'Y')
                 rcs_drawFilledBox(display,vdp->window,vdp->colors[cl],x*12+(px<<1),y*16+(py<<1),2,2);
               else
                 rcs_drawPoint(display,vdp->window,vdp->colors[cl],x*6+px,y*8+py);
               b = (b << 1) & 0xff;
               }
             }
           break;
