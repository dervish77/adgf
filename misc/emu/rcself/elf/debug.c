...
void debug_help() {
  debugDisplay("?                       - Show help");
  debugDisplay("AS addr                 - Assemble");
  debugDisplay("DA [addr][,cnt]         - DisAssemble");
  debugDisplay("DM [addr][,cnt]         - Display Memory");
  debugDisplay("DP                      - Display current memory Pointer");
  debugDisplay("RB addr                 - Remove Breakpoint");
  debugDisplay("RN [addr]               - Run.  addr as optional start address");
  debugDisplay("SB addr                 - Set breakpoint");
  debugDisplay("SM addr byte byte ...   - Set Memory to byte list");
  debugDisplay("SR reg value            - Set Register (P,X,D,DF,T,Q,R0-RF)");
  debugDisplay("ST [num]                - Step 1 or num instructions");
  debugDisplay("TR ON | OFF             - Turn tracing on or off");
  debugDisplay("TRAP asm_cmd            - Set command trap");
  debugDisplay("RT asm_cmd              - Remove command trap");
  debugDisplay("TREG reg value          - Trap on register value");
  debugDisplay("RR reg value            - Remove a register trap");
  }
  ...
  if (event.type == EVENT_KEY_PRESS) {
    ...
    if (event.d1 == 13) {
      ...
      if (debugAsm == 'Y') {
        ...
        if (strcmp(debugLine,"?") == 0) debug_help();
