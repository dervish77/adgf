...
char InFileName[80];		   /* Input Srec file name              */
char OutFileName[80];		   /* Output 'C' file name              */
char ArrayName[80];
...
/* Function Prototypes */
unsigned ParseSFile();
unsigned LineIn(FILE *infile, char *deststr);
int HelpScreen();
...
unsigned ParseSFile()
{
  ...
  fprintf(outfile,"\n\n/* ***********************************************************");
  fprintf(outfile,"\n * Total Number of Srecord Chars: %lu", NumBytesCopied);
  fprintf(outfile,"\n * Number of Srecord Lines:       %lu", NumLinesCopied);
  fprintf(outfile,"\n * Longest Srecord Line:          %lu chars", MaxChars);
  fprintf(outfile,"\n * ***********************************************************");
  fprintf(outfile,"\n */\n\n\n");
...
int HelpScreen()
{
  printf("\nUsage: srectoc -H -I infile -O outfile -A arrayname\n");
  return(1);
}
