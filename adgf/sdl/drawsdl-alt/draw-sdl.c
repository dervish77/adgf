...
int drawInit(int screen_x, int screen_y, int bpp)
{
    ...
    {
	    printf("drawInit: Unable to initialize SDL: %s\n", SDL_GetError());
	    return(1);
    }
    ...
    if (screen == NULL) 
    {
	    printf("drawInit: Unable to set video mode: %s\n", SDL_GetError());
	    return(1);
    }
...
void drawLoadBitmap(char *file, tsImage *bitmap)
{
    SDL_Surface *image;
    SDL_Rect dest;
    ...
    /* Copy image details into bitmap structure */
    bitmap->image  = image;
    bitmap->width  = image->w;
    bitmap->height = image->h;
}

void drawBitmapImage(tsImage *bitmap, int x, int y)
{
    SDL_Rect dest;
    ...
    dest.x = x;
    dest.y = y;
    dest.w = bitmap->image->w;
    dest.h = bitmap->image->h;
    SDL_BlitSurface(bitmap->image, NULL, screen, &dest);
...
void drawCreateBitBlitImage(tsImage *blit, int width, int height, int bpp)
{
    // assumes 16 bit Blt
    SDL_Surface *image;

#ifdef DEBUG
    printf("drawCreateBitBlitImage: w, h, bpp = %d %d %d\n", width, height, bpp);
#endif

    image = SDL_CreateRGBSurface(SDL_SWSURFACE,width,height,bpp,0,0,0,0);
//    image->w = width;
//    image->h = height;
...
void drawFillBitBlitImage(tsImage *blit, unsigned short color)
{
    // assumes 16 bit Blt
    int x,y;
    Uint16 *blit_pixels;
...
#ifdef DEBUG
    printf("drawFillBitBlitImage: filling with color = 0x%X\n", color);
#endif
    ...
    blit_pixels = (Uint16 *) blit->image->pixels;
    ...
    for (y = 0; y < height; y++) 
    {
        ...
        {
	        int offset;
	        offset = ((blit->image->pitch / 2) * y + x);
	        *(blit_pixels+offset) = color;
	    }
    }
}

void drawBitBlitImage(tsImage *blit, int tlx, int tly)
{
    SDL_Rect dest;
    ...
    dest.w = blit->width;
    dest.h = blit->height;
    SDL_BlitSurface(blit->image, NULL, screen, &dest);
...
int drawAddTimer(int delay, int eventId)
{
    ...
    switch(eventId)
    {
        ...
        default:
            timer_id = -1;
            printf("drawAddTimer: Unknown timer requested!\n");
            break;
    }
    return(timer_id);
}
...
{
    fprintf(stderr, "drawDashedLine: not implemented!!\n");
}
