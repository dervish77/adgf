...
int CountBoard(GAME_S_T *pGame)
{
    ...
    winner = winner || CountDiagnonal(pGame, DIAG_LEFT);
    winner = winner || CountDiagnonal(pGame, DIAG_RIGHT);
...
int CountDiagnonal(GAME_S_T *pGame, int diag)
{   
    int winner = 0;
    int p1count = 0;
    int p2count = 0;
...
void HandleMoveRandom(GAME_S_T *pGame)
{
    ...
    if (pGame->move_count > 0)
    {
        i = int_random( 1, pGame->move_count );
#ifndef WIN32 
