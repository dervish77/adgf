...
CalcDistance2Corner(MyProgram *data, int r, int c)
{
   ...
   /* distance to 0,0 */
   temp = ((r) * (r)) + ((c) * (c));
   dist[0] = SQRT_FUNC(temp);

   /* distance to 0,C */
   temp = ((r) * (r)) + ((c - data->screen.max_grid_col) * (c - data->screen.max_grid_col));
   dist[1] = SQRT_FUNC(temp);

   /* distance to R,0 */
   temp = ((r - data->screen.max_grid_row) * (r - data->screen.max_grid_row)) + ((c) * (c));
   dist[2] = SQRT_FUNC(temp);

   /* distance to R,C */
   temp = ((r - data->screen.max_grid_row) * (r - data->screen.max_grid_row)) 
		+ ((c - data->screen.max_grid_col) * (c - data->screen.max_grid_col));
   dist[3] = SQRT_FUNC(temp);
