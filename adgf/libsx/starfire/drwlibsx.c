 ...
 */
COLOR_RGB_T default_colors[MAX_COLOR_MAP] = { 0,   0,   0,
					      41,  158, 147,
					      63,  172, 243, 
					      61,  108, 255,
					      0,   0,   255,
...
*****************************************************************************/
void
EnableNextAnimFrame(MyProgram *data)
{
   AddTimeOut(data->anim.frame_interval, 
		data->anim.callback, (void *)data);
}
...
DrawScore(MyProgram *data)
{
   ...
   /*  display score 
    */
   sprintf( string, "Score - %06d", data->score.score );
   DrawText( string, data->score.score_label.x, data->score.score_label.y );

   /*  display health 
    */
   sprintf( string, "Health - %03d", data->score.health );
   DrawText( string, data->score.health_label.x, data->score.health_label.y );

   /*  display ammo 
    */
   sprintf( string, "Ammo - %03d", data->score.ammo );
   DrawText( string, data->score.ammo_label.x, data->score.ammo_label.y );

   /*  display warp
    */
   if (data->score.warp)
   {
      DrawText( "W", data->score.warp_label.x, data->score.warp_label.y );
   }
   else
   {
      DrawText( " ", data->score.warp_label.x, data->score.warp_label.y );
   }

   /*  display slammer
    */
   if (data->score.slammer)
   {
      DrawText( "S", data->score.slammer_label.x, data->score.slammer_label.y );
   }
   else
   {
      DrawText( " ", data->score.slammer_label.x, data->score.slammer_label.y );
   }
}
...
PopupWinner(MyProgram *data)
{
   ...
   sprintf(string, 
	...
	data->data.player.winning_player);
...
PopupMultiPlayer(MyProgram *data)
{
   ...
   ip = GetString( "Enter IP address of StarFire server\n", NULL );
   if (ip == (char *) NULL)
      return;
...
PopupNewGame(MyProgram *data)
{
   ...
   {
      UpdateStatus( "Resuming current game", data );
   }
}
...
******************************************************************************/
void
PopupInventory(MyProgram *data)
{
   int okay;
   char string[50];
   char inv_text[512];

   sprintf(inv_text, "Inventory\n");

   sprintf(string,    "---------\n\n");
   strcat(inv_text, string); 
   ...
   sprintf(string,"ammo       = %d\n", data->score.ammo);
   strcat(inv_text, string); 

   sprintf(string,"warp       = %d\n", data->score.warp);
   strcat(inv_text, string); 

   sprintf(string,"slammer    = %d\n", data->score.slammer);
   strcat(inv_text, string); 

   sprintf(string,"\n"); 
   strcat(inv_text, string); 

   okay = GetYesNo(inv_text);
}
...
PopupHelpMenu(void)
{
   ...
   sprintf(help_text, "HELP StarFire - Menus\n");
...
PopupHelpKeys(void)
{
   ...
   sprintf(help_text, "HELP StarFire - Keys\n");
   ...
   sprintf(string,"Spacebar        - fire photons\n"); 
   strcat(help_text, string); 

   sprintf(string,"w               - space warp\n\n"); 
   strcat(help_text, string); 
...
PopupHelpRules(void)
{
   ...
   sprintf(help_text, "HELP StarFire - Rules\n");
