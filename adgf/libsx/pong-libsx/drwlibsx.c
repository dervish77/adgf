...
PopupHelpMenu(void)
{
   ...
   sprintf(help_text, "HELP %s - Menus\n", PROGNAME);
...
PopupHelpRules(void)
{
   ...
   sprintf(help_text, "HELP %s - Rules\n", PROGNAME);
...
PopupMultiPlayer(MyProgram *data)
{
   ...
   {
      sprintf(string, 
	"Sorry, no network support in this version of %s.\n\n", PROGNAME);
      okay = GetYesNo(string);
      return;
   }
   ...
   else 
   {
      ...
      sprintf(string, "Enter IP address of %s server\n", PROGNAME);
      ip = GetString( string, NULL );
      if (ip == (char *) NULL)
         return;
