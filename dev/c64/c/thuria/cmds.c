...
void HandleHelp()
{
   ...
   printf("  look        - repeat room\n");
#ifdef SUPPORT_VB_TOGGLE
...
#endif
   printf("  diagnose    - show health\n");
   printf("  n,s,e,w,u,d - travel dir\n");

   printf("\nOther verbs:\n");

   printf("  take, drop, enter, exit, open,\n");
   printf("  close, read, examine, on, off\n");
...
void HandleSwim(GAME_S_T *game)
{
   ...
   switch(cr)
   {
      ...
      case RM_BEAVER_POND:
         printf("\nFar too large for swimming.\n");
         break;

      case RM_MISTY_VALLEY:
         printf("\nFar too small for swimming.\n");
         break;

      case RM_BROOK:
         printf("\nRefreshin but pointless.\n");
         break;
      ...
      case RM_NARROW_CAVE:
         switch(map[RM_LARGE_CAVE].special)
         {
            case SE_BRIDGE_NO:
               ...
               break;
            case SE_BRIDGE_NO2:
               printf("\nYou fall into bottomless fissure.\n");
               PlayerDied(game);
               break;
            case SE_BRIDGE_YES:
               printf("\nYou cross. Bridge shatters behind you.\n\n");
...
BOOLEAN_T HandleSpecialDirection(GAME_S_T *game, char dir)
{
   ...
   switch( cr )
   {
      ...
      case RM_THRONE:
         ...
         {
            case SE_SKEL_SITS:
               printf("\nThe skeleton rises to prevent\n");
			   printf("your escape and then attacks.\n");
               map[cr].special = SE_SKEL_ATTACK;
               retval = TRUE;
               break;
            case SE_SKEL_ATTACK:
               printf("\nThe skeleton prevents your escape.\n");
      ...
      case RM_STATUE:
		...
		|| (dir == 's' && map[cr].special == SE_STATUE_SOUTH) )
         {
            printf("\nIndian statue releases an arrow,\nit passes through your heart.\n");
            PlayerDied( game );
         }
         break;
#if 0
         ...
         case RM_STATUE:	/* drop into statue */
            if (obj == OBJ_COINS)
            {
               if (map[game->current_room].special == SE_STATUE_NORTH)
               {
                  ...
                  printf("\nThe indian statue turns to face\nthe southern doorway.\n");
         ...
         if ( strcmp( game->list[1].cmd, "door" ) == 0 )
         {
            switch(map[cr].special)
            {
               case SE_ODOOR_LOCKED:
                  ...
                  break;
               case SE_ODOOR_CLOSED:
                  printf("\nDoor swings open with a creaking sound.\n");
   ...
   switch(cr)
   {
      case RM_OUTSIDE_MAIN:
         if ( strcmp( game->list[1].cmd, "door" ) == 0 )
         {
            ...
            {
               case SE_ODOOR_OPEN:
                  printf("\nDoor swings closed with a creaking sound.\n");
		...
		|| strcmp( game->list[1].cmd, "padlock" ) == 0 )
         {
            if ( HaveObject(game, OBJ_NAIL, &index) )
            {
               ...
               {
                  case SE_ODOOR_LOCKED:
                     printf("\nAfter some effort, padlock clicks open.\n");
                     map[cr].special = SE_ODOOR_CLOSED;
