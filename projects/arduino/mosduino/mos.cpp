 ...
 *  GLOBALS
 */
CONTEXT_S_T staticContextMemory;
...
int mosInitTasking(void)
{
    ...
    // use statically allocated context block
    context = &staticContextMemory;
    ...
    else
    {
        ...
        gbContext->maxtaskmem = MAX_TASK_MEM_SIZE;
        gbContext->maxmsgcount = MAX_MSG_COUNT;
        gbContext->maxmsgsize = MAX_MSG_SIZE;
        retval = TRUE;
    
        /* create idle task */
        taskid = mosCreateTask("idle", (void *) &idle, NULL, 1);
    ...
    // init task timer
    hwInitTaskTimer(gbContext->ticktime);
...
/*  mosSetVerboseState		- enable/disable verbose debud output
 *
 *  Parameters:
...
void mosDisplayErrorCode(int code)
{
    switch (code)
    {
        ...
        case ERR_TASK_LIST_IS_FULL:
            ...
            break;
        case ERR_DEVICE_OPEN_FAIL:
            mosDbgPrint( 0, "Error code - ERR_DEVICE_OPEN_FAIL\n");
            break;
        case ERR_DEVICE_READ_FAIL:
            mosDbgPrint( 0, "Error code - ERR_DEVICE_READ_FAIL\n");
            break;
        case ERR_DEVICE_WRITE_FAIL:
            mosDbgPrint( 0, "Error code - ERR_DEVICE_WRITE_FAIL\n");
            break;
        case ERR_DEVICE_CLOSE_FAIL:
            mosDbgPrint( 0, "Error code - ERR_DEVICE_CLOSE_FAIL\n");
            break;
        case ERR_READ_MSG_INDEX_ERROR:
            mosDbgPrint( 0, "Error code - ERR_READ_MSG_INDEX_ERROR\n");
            break;
        default:
...
int mosCreateTask(char *name, void *proc, void *args, int size)
{
    ...
    else if (gbContext->numtasks < gbContext->maxtasks)
    {
        ...
        // calculate new taskid from previous taskid
        taskid = gbContext->tasklist[index - 1].taskid + 1;
        // save new taskid for new task
        gbContext->tasklist[index].taskid = taskid;
    }
    else
    {
    ...
    // save other task parameters
    strncpy(gbContext->tasklist[index].taskname, name, SHORT_BUFF_LEN);
    gbContext->tasklist[index].proc = (int (*)(void *)) (proc);
    ...
    // allocate new memblock for this task
    if (size > 0 && size <= gbContext->maxtaskmem)
    {
        gbContext->tasklist[index].mem.size = size;
        // use pointer to statically allocated task memory buffer
        gbContext->tasklist[index].mem.ptr = (char *) &gbContext->tasklist[index].mem.buf[0];
    ...
    else if (size > gbContext->maxtaskmem)
    {
        if (gbContext->verbose) mosDbgPrint( 0, "MOS: too much task memory requested (%d)!!\n", size);
        gbErrorCode = ERR_ALLOC_MEM_FAIL;
        return (TASK_CREATE_FAIL);
    }
...
int mosIsTaskBlocked(int taskid)
{
    ...
    if (index != INVALID_TASK_INDEX)
    {
        return (gbContext->tasklist[index].blocked);
    }
    else
    {
        gbErrorCode = ERR_INVALID_TASK_INDEX;
