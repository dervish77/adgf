...
by editing the game data files (map.c, room.c, object.c, and special.c).  
...
To unpack the game (Windows version):

	make a directory called "thuria_src"

	unzip "thuria.zip" into the directory "thuria_src"


To install the game (from Win95):

	execute the install script "thuria_src\win\install.bat"
	

To run the game (from Win95):

	Start->Programs->Thuria->Dungeons_of_Thuria


To uninstall the game (from Win95):

	execute the uninstall script "thuria_src\win\uninstall.bat"


__________________________________________________________________________

Running the game (Windows version from MSDOS shell):
